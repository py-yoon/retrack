...
export default function LoginModal() {
  const { isLoginModalOpen, closeLoginModal, signInWithKakao, signInWithGoogle, loginRedirectUrl } = useAuth();
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={closeLoginModal}
            className="rounded-full p-1 text-[#777a76] hover:bg-black/5 hover:text-black"
            aria-label="닫기"
          >
        ...
        <p className="mt-2 text-sm leading-relaxed text-[#6e716e]">
          관심 있는 정비사업장을 등록하고 변동 공고를 한곳에서 편리하게 모아보세요.
        </p>
        ...
        <div className="mt-6 flex flex-col gap-3">
          ...
          <button
            ...
            className="flex h-12 w-full items-center justify-center gap-3 rounded-2xl bg-[#FEE500] font-semibold text-[#191919] transition hover:brightness-95 active:scale-[0.99]"
          >
          ...
          <button
            ...
            className="flex h-12 w-full items-center justify-center gap-3 rounded-2xl border border-black/10 bg-white font-semibold text-[#374151] transition hover:bg-[#f9f9f9] active:scale-[0.99]"
          >
        ...
        <p className="mt-6 text-center text-xs leading-relaxed text-[#989b96]">
          별도의 비밀번호 생성이나 개인정보 입력 없이 안전하게 인증되며, 관심 사업장 저장 및 알림 목적으로만 식별됩니다.
        </p>
      </div>
    </div>
  );
